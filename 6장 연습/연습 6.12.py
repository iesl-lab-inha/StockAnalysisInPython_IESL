...
class DualMomentum:
    ...
    def get_rltv_momentum(self, start_date, end_date, stock_count):
        ...
        connection = pymysql.connect(host='localhost', port=3306,
                                     db='INVESTAR', user='root', passwd='0000', autocommit=True)
        ...
        if (result[0] is None):
            ...
            return
        start_date = result[0].strftime('%Y-%m-%d')     # 2
        ...
        for _, code in enumerate(self.mk.codes):
            ...
            if (result is None):
                continue
            new_price = int(result[0])              # 5
            returns = (new_price / old_price - 1) * 100     # 6
        ...
        df = df.sort_values(by='returns', ascending=False)      # 9
        df = df.head(stock_count)
        df.index = pd.Index(range(stock_count))     # 10
